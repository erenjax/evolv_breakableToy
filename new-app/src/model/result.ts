...
export type DigitResult = {
  kind: "NotChecked" | "CheckedRightDigit" | "CheckedWrongDigit" | "RightDigitWrongPlace"
...
export const buildResult = (code: Code.Code, guess: Code.Code): Result => {
  return [
    {kind: "NotChecked", usedByOtherDigit: false, codeValue: code[0], guessValue: guess[0]},
    {kind: "NotChecked", usedByOtherDigit: false, codeValue: code[1], guessValue: guess[1]},
    {kind: "NotChecked", usedByOtherDigit: false, codeValue: code[2], guessValue: guess[2]},
    {kind: "NotChecked", usedByOtherDigit: false, codeValue: code[3], guessValue: guess[3]}]
}
...
const checkRightDigitWrongPlace1 = (result: Result) => (digitResult: DigitResult, index: number): DigitResult => {
  let foundWRDP = false
  const nextDigitResult: DigitResult = {...digitResult}
  if (digitResult.kind === "CheckedRightDigit" || digitResult.kind === "RightDigitWrongPlace") {
...
export const checkResult = (code: Code.Code, guess: Code.Code): Result => {
  ...
  const checkedForRightDigitWrongPlace = checkRightDigitWrongPlace(checkedForRightDigits)
...
export const checkResultLight = (code: Code.Code, guess: Code.Code): string[] => {
